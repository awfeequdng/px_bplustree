...
#ifdef NDEBUG
#define PXBPT_ASSERT(expr, message) ((void)0)
#else
/*
 ...
 */
#define PXBPT_ASSERT(expr, message) assert((expr) && (message))
#endif /* NDEBUG */
...
public:
    ...
    public:
        ...
        void PushBack(const ElementType *copy_start_p, const ElementType *copy_end_p) {
            ...
            PXBPT_ASSERT(copy_start_p <= copy_end_p, "Loop will not come to an end.");
    ...
    public:
        ...
        ElementType &At(const int index) {
            ...
            PXBPT_ASSERT(index < GetSize(), "Index out of range.");
    ...
    public:
        ...
        KeyValuePair *FindLocation(const KeyType &element, BPlusTree *tree) {
            ...
            KeyValuePair *iter = std::lower_bound(
                this->Begin(), this->End(), dummy_element,
                [tree](const auto &a, const auto &b) { return !tree->KeyCmpGreater(a.first, b.first); });
            return iter;
        }
...
public:
    ...
    public:
        ...
        KeyType Key() {
            PXBPT_ASSERT(curr_key_ != nullptr, "Iterator should be valid, pointing to a valid key.");
         ...
         */
        void operator++() {
            PXBPT_ASSERT(state_ == VALID, "Iterator in Invalid State.");
    ...
    //                    bool high_key_exists, uint32_t limit, std::vector<ValueType> *value_list,
     ...
     */
    bool ScanDescending(KeyType index_low_key, KeyType index_high_key, std::vector<ValueType> *value_list) {
     ...
     * @return true on success, false on failure
     */
    bool ScanLimitDescending(KeyType index_low_key, KeyType index_high_key, std::vector<ValueType> *value_list,
                             uint32_t limit, std::function<bool(const ValueType)> predicate) {
        BPlusTreeIterator iterator = RBegin(index_high_key);

        while ((value_list->size() < limit) && KeyCmpGreaterEqual(iterator.Key(), index_low_key) &&
               (iterator != REnd()) && (iterator != Retry())) {
            if (!predicate(iterator.Value())) {
                --iterator;
                continue;
            }
    ...
    bool Insert(const KeyElementPair element, std::function<bool(const ValueType)> predicate) {
        ...
        */
        ...
        if (!finished_insertion) {
            ...
            if (node->InsertElementIfPossible(key_list_value,
                ...
                if (node->GetElasticHighKeyPair()->second != nullptr) {
                    // Get exclusive access on right node to set its linksGetElasticHighKeyPair
        ...
        if (!finished_insertion) {
            PXBPT_ASSERT(got_root_latch, "Root Latch should be held here");
            auto old_root = root_;
            KeyNodePointerPair p1, p2;
            ...
            root_ =
                ElasticNode<KeyNodePointerPair>::Get(inner_node_size_upper_threshold_, NodeType::InnerType,
            ...
            if (is_deleted) {
                if (!lock_list->empty()) {
                    if (child_pointer->GetType() == NodeType::LeafType) {
                        ...
                        DeleteRebalance<KeyNodePointerPair>(node, child_pointer, index,
                                                            GetInnerNodeSizeLowerThreshold());
                    }
                }
